...
BEGIN 
	...
	BEGIN TRY
		...
		UPDATE dbo.Zakaz 
            ...
            Nachalo_zakaza_data = GETDATE(), LAST_STATUS_TIME = GETDATE()
...
BEGIN 
	...
	REMOTE_SYNC = (CASE WHEN (@driver_id > 0) THEN 1 ELSE 0 END),
        ...
        Data_predvariteljnaya = (CASE WHEN (@shedule_date IS NULL OR @online_sheduled_as_lock_prev <> 1) THEN GETDATE() ELSE @shedule_date END),
        is_early = (CASE WHEN (@shedule_date IS NULL OR @online_sheduled_as_lock_prev = 1) THEN 0 ELSE 1 END),
        early_date = (CASE WHEN (@shedule_date IS NULL OR @online_sheduled_as_lock_prev = 1) THEN GETDATE() ELSE @shedule_date END),
